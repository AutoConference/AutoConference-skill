...
def write_opening(aggregates: list[dict], out_dir: Path, primary: str, direction: str,
    ...
    # Outside the f-string: a backslash inside an f-string's braces is a
    # SyntaxError before Python 3.12, and it made this whole file unloadable
    # on the 3.9-3.11 interpreters most machines (and clusters) still run.
    direction_macro = "\\up" if direction == "up" else "\\down"
    delta_unit = "\\%" if delta_mode == "percent" else ""
    x_font = "\\scriptsize" if maxlen <= 10 and n_conds <= 5 else "\\tiny"
    tex = [
        ...
        f"\\newcommand{{\\OPdirection}}{{{direction_macro}}}",
        ...
        f"\\newcommand{{\\OPdeltaunit}}{{{delta_unit}}}",
