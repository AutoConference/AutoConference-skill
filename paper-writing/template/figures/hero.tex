...
\begin{center}
  % Shrink only. The grid below is drawn at a fixed ~13cm, so a plain
  % \resizebox{0.99\linewidth} ENLARGED it by 6% in the single-column
  % layouts and the teaser's type came out bigger than the body text.
  % Same bug the table grammars had. In cvpr2 the column is narrower than
  % 13cm and this still shrinks, which is what it was there for.
  \resizebox{\ifdim\width>0.99\linewidth 0.99\linewidth\else\width\fi}{!}{%
